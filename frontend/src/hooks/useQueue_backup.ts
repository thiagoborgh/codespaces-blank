...
export const useQueue = () => {
  ...
  const getFilteredPatients = useCallback((
    ...
    if (hasSearchTerm) {
      ...
      if (statusFilter && statusFilter !== 'all' && statusFilter !== 'undefined') {
        ...
        filtered = filtered.filter((p: QueuePatient) => {
          ...
          if (!shouldInclude) {
            console.log(`❌ Filtro de status "${statusFilter}" removeu: ${p.name} (status: ${p.status})`);
          }
          return shouldInclude;
        });
        console.log('Após filtro por status:', { statusFilter, beforeFilter: beforeStatusFilter, afterFilter: filtered.length });
      }
    }
    ...
    if (filters) {
      console.log('🔄 Aplicando filtros avançados:', { filters, beforeFilter: filtered.length, hasSearchTerm });
      ...
      if (filters.status.length > 0 && !hasSearchTerm) {
        ...
        console.log('Filtro avançado - status:', { beforeFilter: beforeStatusFilter, afterFilter: filtered.length, statusFilter: filters.status });
      } else if (filters.status.length > 0 && hasSearchTerm) {
        console.log('⚠️ Filtro avançado de status ignorado devido à busca ativa');
      }
      ...
      if (filters.serviceTypes.length > 0) {
        ...
        console.log('Filtro avançado - tipo de serviço:', { beforeFilter: beforeServiceFilter, afterFilter: filtered.length, serviceTypesFilter: filters.serviceTypes });
      }
      ...
      if (filters.teams.length > 0) {
        ...
        console.log('Filtro avançado - equipe:', { beforeFilter: beforeTeamFilter, afterFilter: filtered.length, teamsFilter: filters.teams });
      }
      ...
      if (filters.professionals.length > 0) {
        ...
        console.log('Filtro avançado - profissional:', { beforeFilter: beforeProfessionalFilter, afterFilter: filtered.length, professionalsFilter: filters.professionals });
      }
      ...
      if (filters.onlyUnfinished && !hasSearchTerm) {
        ...
        console.log('Filtro avançado - não finalizados:', { beforeFilter: beforeUnfinishedFilter, afterFilter: filtered.length });
      } else if (filters.onlyUnfinished && hasSearchTerm) {
        console.log('⚠️ Filtro "não finalizados" ignorado devido à busca ativa');
      }
    }
    ...
    if (sortBy && sortBy !== 'arrival') {
      ...
      console.log('Após ordenação:', { sortBy, filteredLength: filtered.length });
    } else {
      // Manter ordenação padrão se não especificado
      filtered = applyDefaultSorting(filtered);
    }

    console.log('Resultado final do filtro:', { filteredLength: filtered.length });
    return filtered;
  ...
  // Marcar paciente como "não aguardou"
  const markPatientAsNoShow = useCallback(async (patientId: number) => {
    ...
    try {
      ...
      setPatients(prev => {
        const updated = prev.map((p: QueuePatient) => 
          p.id === patientId 
